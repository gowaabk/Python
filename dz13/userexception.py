...
class UserMinLengthError(UserExceptions):
...
class UserTriangleError(UserExceptions):
    ...
    def __str__(self):
        print(
            f"Tреугольника со сторонами {self.a}, {self.b}, {self.c} не существует!")


class CheckBagSize(UserExceptions):
    ...
    def __str__(self):
        print(
            f"Предмет размером {self.item} не влезет в сумку размером {self.bag}!")
